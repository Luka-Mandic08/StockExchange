...
public interface StockOrderRepository extends JpaRepository<StockOrder, Integer> {
    ...
    @Query("select o from StockOrder o where (o.stockOrderType = 0 and o.stockOrderStatus <= 1 and o.price >= ?1) order by o.price desc, o.submissionDate asc")
    public List<StockOrder> findFittingBuyingStockOrders(double priceLimit);

    @Query("select o from StockOrder o where (o.stockOrderType = 1 and o.stockOrderStatus <= 1 and o.price <= ?1) order by o.price asc, o.submissionDate asc")
    public List<StockOrder> findFittingSellingStockOrders(double priceLimit);
}
